...
/**
 * <h2>JsonExtensions â€” Advanced Search and Transformation Utilities</h2>
 *
 * <p>
 * Provides wildcard path search, deep key/value search, schema inference,
 * FIX protocol parsing, and self-healing parser capabilities.
 * </p>
 *
 * <h3>Examples:</h3>
 * 
...
public class JsonExtensions {
     ...
     * <ul>
     * <li>{@code store.books[*].title} â€” all titles in books array</li>
     * <li>{@code store.*.price} â€” all price values in any child of store</li>
     * <li>{@code *} â€” all direct children of root</li>
     * </ul>
     *
            ...
            for (JsonNode n : current) {
                if (segment.equals("*")) {
                    if (n instanceof JsonObject obj)
                        next.addAll(obj.fields().values());
                    else if (n instanceof JsonArray arr)
                        next.addAll(arr.elements());
    ...
    /**
     * Parses a FIX Protocol (tag=value|tag=value) message into a
     * {@link JsonObject}.
    ...
    public JsonNode fixToJson(String fixMessage) {
        ...
        for (String pair : fixMessage.split("\\|")) {
            ...
            if (kv.length == 2)
                obj.put("tag_" + kv[0].trim(), new JsonValue(kv[1].trim()));
        }
        return obj;
    }
    ...
    public JsonNode parseSelfHealing(String rawJson) {
        ...
        try {
            return io.github.dhoondlayai.artifact.json.streaming.FastJsonEngine.parse(healed);
        } catch (Exception e) {
