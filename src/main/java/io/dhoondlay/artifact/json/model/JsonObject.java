...
/**
 * <h2>JsonObject — High-Performance JSON Object Node</h2>
 *
 * <p>A mutable, ordered JSON object backed by a {@link LinkedHashMap} for O(1) lookups
 * while preserving insertion order (important for deterministic serialization).</p>
 *
 * <p>All mutating methods return {@code this} for fluent chaining. All read operations
 * are fully Stream-API compatible.</p>
 *
...
public final class JsonObject implements JsonNode {
    ...
    /** Convenience overload — wraps a raw {@link String}. */
    public JsonObject put(String key, String value) {
        return put(key, new JsonValue(value));
    }

    /** Convenience overload — wraps a raw {@link Number}. */
    public JsonObject put(String key, Number value) {
        return put(key, new JsonValue(value));
    }

    /** Convenience overload — wraps a raw {@link Boolean}. */
    ...
    public JsonObject rename(String oldKey, String newKey) {
        ...
        if (val != null) data.put(newKey, val);
        return this;
    }
    ...
    public Optional<Integer> getInt(String key) {
        ...
        return (n instanceof JsonValue v && v.value() instanceof Number num)
                ? Optional.of(num.intValue()) : Optional.empty();
    }
    ...
    public Optional<Long> getLong(String key) {
        JsonNode n = data.get(key);
        return (n instanceof JsonValue v && v.value() instanceof Number num)
                ? Optional.of(num.longValue()) : Optional.empty();
    }
    ...
    public Optional<Double> getDouble(String key) {
        JsonNode n = data.get(key);
        return (n instanceof JsonValue v && v.value() instanceof Number num)
                ? Optional.of(num.doubleValue()) : Optional.empty();
    }
    ...
    public Map<String, Object> toMap() {
        ...
        data.forEach((k, v) -> {
            if (v instanceof JsonValue val) result.put(k, val.value());
        });
        return result;
    }
    ...
    String toPrettyInternal(int indent, int depth) {
        ...
        for (var e : data.entrySet()) {
            ...
            if (e.getValue() instanceof JsonObject o) sb.append(o.toPrettyInternal(indent, depth + 1));
            else if (e.getValue() instanceof JsonArray a) sb.append(a.toPrettyInternal(indent, depth + 1));
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof JsonObject other)) return false;
