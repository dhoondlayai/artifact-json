...
/**
 * <h2>JsonArray — High-Performance JSON Array Node</h2>
 *
 * <p>An ordered, mutable JSON array backed by an {@link ArrayList}. Offers full
 * Java Stream API support including parallel processing for large datasets.</p>
 *
 * <h3>Why faster than Jackson / org.json?</h3>
 * <ul>
 *   <li>Sealed type hierarchy — JVM can devirtualize method calls</li>
 *   <li>No ObjectMapper or schema needed — direct tree access</li>
...
public final class JsonArray implements JsonNode, Iterable<JsonNode> {
    ...
    /** Wraps an existing list as the internal backing — no copy. */
    ...
    /** Convenience overload — wraps a {@link String}. */
    public JsonArray add(String value)  { return add(new JsonValue(value)); }
    /** Convenience overload — wraps a {@link Number}. */
    public JsonArray add(Number value)  { return add(new JsonValue(value)); }
    /** Convenience overload — wraps a {@code boolean}. */
    public JsonArray add(boolean value) { return add(new JsonValue(value)); }
    /** Appends a JSON null. */
    public JsonArray addNull()          { return add(new JsonValue(null)); }
    ...
    public JsonNode elementOrDefault(int index, JsonNode defaultValue) {
        if (index < 0 || index >= data.size()) return defaultValue;
        return data.get(index);
    }
    ...
    /**
     * Returns {@code true} if the given node is present (uses deep structural equals).
     *
    ...
    /**
     * Returns a new {@link JsonArray} with elements from {@code fromIndex} (inclusive)
    ...
    /**
     * Returns a page of elements — useful for pagination.
     *
    ...
    public JsonArray page(int page, int pageSize) {
        ...
        if (from >= data.size()) return new JsonArray();
    ...
    public Stream<JsonNode> stream() { return data.stream(); }

    /**
     * Returns a parallel {@link Stream} of elements.
     * Use for large arrays where each element can be processed independently.
     */
    public Stream<JsonNode> parallelStream() { return data.parallelStream(); }

    /** Iterates all elements. */
    @Override
    public Iterator<JsonNode> iterator() { return data.iterator(); }
    ...
    /**
     * Maps every element using the given function, producing a new {@link JsonArray}.
     *
    ...
    /**
     * Filters elements matching the given predicate, returning a new {@link JsonArray}.
     *
    ...
    public JsonArray distinct() {
        ...
        JsonArray result = new JsonArray();
        for (JsonNode n : data) {
            if (seen.add(n.toString())) result.add(n);
        }
        return result;
    }
     ...
     * // grouped.get("admin") → JsonArray of admins
     * }</pre>
     *
     * @param fieldKey the field to group by
     * @return a {@link Map} from group key → {@link JsonArray} of matching elements
     */
    ...
    public double sum(String field) {
        return stream()
                ...
                .mapToDouble(n -> ((JsonObject) n).field(field) != null
                        ? ((JsonObject) n).field(field).asDouble() : 0.0)
                .sum();
    }
    ...
    public OptionalDouble min(String field) {
        return stream()
                .filter(n -> n instanceof JsonObject)
                .mapToDouble(n -> ((JsonObject) n).field(field) != null
                        ? ((JsonObject) n).field(field).asDouble() : Double.MAX_VALUE)
                .min();
    }
    ...
    public OptionalDouble max(String field) {
        return stream()
                .filter(n -> n instanceof JsonObject)
                .mapToDouble(n -> ((JsonObject) n).field(field) != null
                        ? ((JsonObject) n).field(field).asDouble() : Double.MIN_VALUE)
                .max();
    }
    ...
    String toPrettyInternal(int indent, int depth) {
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            if (n instanceof JsonObject o) sb.append(o.toPrettyInternal(indent, depth + 1));
            else if (n instanceof JsonArray a) sb.append(a.toPrettyInternal(indent, depth + 1));
            else sb.append(n);
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof JsonArray other)) return false;
