...
/**
 * <h2>JsonStreamWriter — Zero-Allocation Event-Based JSON Writer</h2>
 *
 * <p>A pull-based JSON generator inspired by Jackson's {@code JsonGenerator}.
 * Writes JSON tokens directly to an {@link OutputStream} or {@link Writer}
 * without building an in-memory {@link io.dhoondlay.artifact.json.model.JsonNode} tree.
 * This is the recommended approach for serializing very large datasets.</p>
 *
 * <h3>When to use JsonStreamWriter:</h3>
 * <ul>
 *   <li>Generating JSON responses for APIs without loading all data in memory</li>
 *   <li>Streaming millions of records from a database to JSON file</li>
 *   <li>Building large JSON arrays incrementally</li>
 * </ul>
 *
 * <h3>Example — streaming array:</h3>
 * <pre>{@code
...
public final class JsonStreamWriter implements Closeable {
    ...
    public JsonStreamWriter endObject() throws IOException {
        depth--;
        if (!firstItem[depth + 1]) newlineAndIndent();
        writer.write('}');
    ...
    private void writeSeparatorAndIndent() throws IOException {
        if (!firstItem[depth]) writer.write(',');
        if (prettyPrint && depth > 0) newlineAndIndent();
    }
    ...
    private void writeStringValue(String s) throws IOException {
        writer.write('"');
        if (s == null) { writer.write("null\""); return; }
        for (int i = 0; i < s.length(); i++) {
            ...
            switch (c) {
                ...
                default   -> {
                    if (c < 0x20) writer.write(String.format("\\u%04x", (int) c));
                    else writer.write(c);
                }
            }
        }
