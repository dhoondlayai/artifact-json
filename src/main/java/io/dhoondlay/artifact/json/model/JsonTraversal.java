...
/**
 * <h2>JsonTraversal — Superfast Tree Traversal Engine</h2>
 *
 * <p>Provides iterative (non-recursive) algorithms for traversing, transforming,
 * and flattening JSON trees. Using an explicit stack/queue instead of recursion
 * ensures there are no {@link StackOverflowError}s even on deeply nested (10,000+
 * level) structures.</p>
 *
 * <h3>Available algorithms:</h3>
 * <ul>
 *   <li>{@link #dfs(JsonNode, Consumer)} — Depth-first (iterative)</li>
 *   <li>{@link #bfs(JsonNode, Consumer)} — Breadth-first (iterative)</li>
 *   <li>{@link #traverse(JsonNode, Consumer)} — Alias for DFS (convenience)</li>
 *   <li>{@link #transform(JsonNode, Function)} — Deeply clone + apply function</li>
 *   <li>{@link #flatten(JsonNode)} — Produce a dot-path → value map</li>
 *   <li>{@link #unflatten(Map)} — Reconstruct a tree from a flat map</li>
 *   <li>{@link #traverseWithPaths(JsonNode, BiConsumer)} — DFS with dot-path context</li>
 * </ul>
 *
 * <h3>Examples:</h3>
 * <pre>{@code
...
public final class JsonTraversal {
    ...
    public static void dfs(JsonNode root, Consumer<JsonNode> consumer) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (current instanceof JsonObject obj) {
                ...
                for (int i = vals.size() - 1; i >= 0; i--) stack.push(vals.get(i));
            } else if (current instanceof JsonArray arr) {
                List<JsonNode> elems = arr.elements();
                for (int i = elems.size() - 1; i >= 0; i--) stack.push(elems.get(i));
            }
        }
    }
     ...
     * Used internally by {@link io.dhoondlay.artifact.json.extensions.JsonExtensions}.
     *
     * @param root     starting node
     * @param consumer receives each visited {@link JsonNode}
     */
     ...
     *   "store.books[0].title"      → title string
     * </pre>
     *
     * @param root     starting node
    ...
    private static void traverseInternal(JsonNode node, String path,
        ...
        if (node instanceof JsonObject obj) {
            obj.fields().forEach((k, v) ->
                traverseInternal(v, path.isEmpty() ? k : path + "." + k, consumer));
    ...
    /**
     * Deep-clones the entire tree while applying a transformation function to each node.
     * The function controls what the node becomes — return the original node unchanged
     ...
     * Array indices are bracketed: {@code store.books[0].title → "The Great Gatsby"}.
     *
     * <h4>Example:</h4>
     * <pre>{@code
     * Map<String, Object> flat = JsonTraversal.flatten(root);
     ...
     *
     * @param flatMap flat path → value map
    ...
    public static List<JsonValue> collectLeaves(JsonNode root) {
        ...
        dfs(root, n -> { if (n instanceof JsonValue v) leaves.add(v); });
        return leaves;
    }
