...
import io.dhoondlay.artifact.json.exception.JsonMappingException;
...
/**
 * <h2>CustomObjectMapper — Bidirectional POJO ↔ JsonNode Mapper</h2>
 *
 * <p>Maps Java objects to {@link JsonNode} trees and back, with full support for:</p>
 * <ul>
 *   <li>{@link JsonProperty} — rename fields in the JSON output</li>
 *   <li>{@link JsonIgnore} — skip fields entirely</li>
 *   <li>{@link JsonNaming} — apply SNAKE_CASE / KEBAB_CASE / PASCAL_CASE automatically</li>
 *   <li>{@link TypeAdapter} — plug in custom serialization logic per type</li>
 *   <li>Collections ({@link java.util.List}, {@link java.util.Set}) → {@link JsonArray}</li>
 *   <li>Maps → {@link JsonObject}</li>
 ...
 *
 * // Serialize POJO → JsonNode
 * JsonNode json = mapper.serialize(myObject);
 *
 * // Deserialize JsonNode → POJO
...
public class CustomObjectMapper {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Serialize: Java Object → JsonNode
    // ─────────────────────────────────────────────────────────────────────────

    /**
     * Converts a Java object to a {@link JsonNode} tree.
     * Handles primitives, Strings, Collections, Maps, and arbitrary POJOs via reflection.
     *
    ...
    private JsonNode doSerialize(Object obj) throws Exception {
        ...
        if (clazz.isArray()) {
            ...
            for (int i = 0; i < len; i++) arr.add(doSerialize(Array.get(obj, i)));
            return arr;
        }
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Deserialize: JsonNode → Java Object
    // ─────────────────────────────────────────────────────────────────────────
    ...
    private <T> T doDeserialize(JsonNode node, Class<T> clazz) throws Exception {
        if (node instanceof JsonValue val) {
            ...
            // Type coercion
            if (clazz == String.class)           return clazz.cast(String.valueOf(raw));
            if (clazz == int.class    || clazz == Integer.class)  return (T)(Integer)((Number)raw).intValue();
            if (clazz == long.class   || clazz == Long.class)     return (T)(Long)((Number)raw).longValue();
            if (clazz == double.class || clazz == Double.class)   return (T)(Double)((Number)raw).doubleValue();
            if (clazz == boolean.class|| clazz == Boolean.class)  return (T)(Boolean)raw;
    ...
    /**
     * Produces a {@link JsonObject} describing the differences between {@code source}
    ...
    public JsonNode diff(JsonNode source, JsonNode target) {
        if (source.equals(target)) return new JsonObject();
