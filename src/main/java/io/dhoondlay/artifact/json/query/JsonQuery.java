package io.dhoondlay.artifact.json.query;

import io.dhoondlay.artifact.json.exception.JsonQueryException;
...
/**
 * <h2>JsonQuery — In-Memory SQL Engine for JSON Arrays</h2>
 *
 * <p>Perform full SQL-like query operations natively on {@link JsonArray} trees
 ...
 * until {@link #execute()} is called.</p>
 *
 * <h3>Supported operations:</h3>
 * <table border="1">
 *   <tr><th>SQL Equivalent</th><th>JsonQuery Method</th></tr>
 *   <tr><td>SELECT col1, col2</td><td>{@link #select(String...)}</td></tr>
 *   <tr><td>SELECT col AS alias</td><td>{@link #selectAs(String, String)}</td></tr>
 *   <tr><td>WHERE col = val</td><td>{@link #whereEq(String, Object)}</td></tr>
 *   <tr><td>WHERE col != val</td><td>{@link #whereNotEq(String, Object)}</td></tr>
 *   <tr><td>WHERE col > val</td><td>{@link #whereGt(String, double)}</td></tr>
 *   <tr><td>WHERE col >= val</td><td>{@link #whereGte(String, double)}</td></tr>
 *   <tr><td>WHERE col &lt; val</td><td>{@link #whereLt(String, double)}</td></tr>
 *   <tr><td>WHERE col &lt;= val</td><td>{@link #whereLte(String, double)}</td></tr>
 *   <tr><td>WHERE col LIKE '%x%'</td><td>{@link #whereContains(String, String)}</td></tr>
 *   <tr><td>WHERE col LIKE 'x%'</td><td>{@link #whereStartsWith(String, String)}</td></tr>
 *   <tr><td>WHERE col LIKE '%x'</td><td>{@link #whereEndsWith(String, String)}</td></tr>
 *   <tr><td>WHERE col REGEXP</td><td>{@link #whereMatches(String, String)}</td></tr>
 *   <tr><td>WHERE col IN (...)</td><td>{@link #whereIn(String, Object...)}</td></tr>
 *   <tr><td>WHERE col IS NOT NULL</td><td>{@link #whereNotNull(String)}</td></tr>
 *   <tr><td>WHERE col IS NULL</td><td>{@link #whereNull(String)}</td></tr>
 *   <tr><td>ORDER BY col</td><td>{@link #orderBy(String, boolean)}</td></tr>
 *   <tr><td>LIMIT n</td><td>{@link #limit(int)}</td></tr>
 *   <tr><td>OFFSET n</td><td>{@link #offset(int)}</td></tr>
 *   <tr><td>PAGE n SIZE s</td><td>{@link #page(int, int)}</td></tr>
 *   <tr><td>DISTINCT</td><td>{@link #distinct()}</td></tr>
 *   <tr><td>GROUP BY col</td><td>{@link #groupBy(String)}</td></tr>
 *   <tr><td>JOIN other ON key</td><td>{@link #join(JsonArray, String, String)}</td></tr>
 *   <tr><td>COUNT / SUM / AVG / MIN / MAX</td><td>{@link #count()}, {@link #sum(String)}, {@link #avg(String)}, {@link #min(String)}, {@link #max(String)}</td></tr>
 * </table>
 *
 * <h3>Examples:</h3>
 * <pre>{@code
...
public final class JsonQuery {
     ...
     *
     * @param array the source array (typically containing {@link JsonObject} elements)
     */
    ...
    /**
     * Projects results to the specified fields only (SQL {@code SELECT col1, col2}).
    ...
    /**
     * Projects a field with an alias in the output (SQL {@code SELECT col AS alias}).
     *
    ...
    /** WHERE field IS NOT NULL — field must be present and non-null. */
    ...
    /** WHERE field IS NULL — field absent or explicitly null. */
    ...
    public JsonQuery thenBy(String field, boolean descending) {
        if (this.comparator == null) return orderBy(field, descending);
    ...
    /** Applies pagination — equivalent to {@code LIMIT size OFFSET page*size}. */
    ...
    /** Uses {@code parallelStream()} for execution — ideal for large arrays. */
    ...
    public JsonArray execute() {
        try {
            ...
            if (offsetSize > 0)     stream = stream.skip(offsetSize);
            if (limitSize  > 0)     stream = stream.limit(limitSize);

            // Build index for JOIN if needed
            Map<String, JsonObject> rightIndex = buildJoinIndex();

            // Collect using sequential collector (parallel used above, collect sequentially)
    ...
    private JsonObject project(JsonObject obj, Map<String, JsonObject> rightIndex) {
        ...
        if (rightIndex != null) {
            ...
            if (leftKeyNode != null) {
                ...
                if (right != null) merged = obj.merge(right);
            }
        }

        // Apply SELECT projection
        if (selectFields.isEmpty()) return merged;
    ...
    private Map<String, JsonObject> buildJoinIndex() {
        ...
        joinArray.stream()
                ...
                .forEach(obj -> {
                    ...
                    if (keyNode != null) index.put(keyNode.asText(), obj);
                });
        return index;
    }

    // ─────────────────────────────────────────────────────────────────────────
    // AGGREGATION (terminal operations — do not need execute())
    // ─────────────────────────────────────────────────────────────────────────
     ...
     * @return map from group value → {@link JsonArray}
     */
